...
import type { DeviceInfo } from '@monitorjs/types'
...
export function getGlobal<T>() {
  if (isBrowserEnv)
    return window as unknown as MITOGlobal & T
  if (isWxMiniEnv)
    return wx as unknown as MITOGlobal & T
  // it's true when run e2e
  if (isNodeEnv)
    return process as unknown as MITOGlobal & T
}
