import { BREADCRUMBCATEGORYS, ErrorTypes, WxBreadcrumbTypes } from '@monitorjs/shared'
import type { WxParsedErrorType } from '@monitorjs/types'
...
export function parseErrorString(str: string): WxParsedErrorType {
  ...
  if (splitLine.length < 2)
    return null
  if (splitLine[0].includes('MiniProgramError'))
